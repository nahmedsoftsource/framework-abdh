...
namespace Superior.MobileMedics.Common
{
  public static class Utility
  {
    ...
    /// <summary>
    /// Try parse date time and validate value with the sql data type
    /// Return true if it is compatible with the sql data type
    /// ortherwise it will return false
    /// </summary>
    /// <param name="InDateTime"></param>
    /// <param name="OutDateTime"></param>
    /// <returns></returns>
    public static bool TryParseDateTime(string InDateTime, out DateTime OutDateTime)
    {
      OutDateTime = new DateTime();
      if (DateTime.TryParse(InDateTime, out OutDateTime))
      {
        if (OutDateTime < System.Data.SqlTypes.SqlDateTime.MinValue.Value ||
          OutDateTime > System.Data.SqlTypes.SqlDateTime.MaxValue.Value)
        {
          return false;
        }
      }
