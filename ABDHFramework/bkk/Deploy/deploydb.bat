...
if exist "%LOGFOLDER%" goto deploydb
mkdir "%LOGFOLDER%"

:deploydb
"%SYSTEMROOT%\Microsoft.NET\Framework\v3.5\MSBuild" Deploy.proj /t:DeployDB /p:LogFolder="%LOGFOLDER%" /l:FileLogger,Microsoft.Build.Engine;logfile="%LOGFOLDER%\deploydb_%yymmdd_hhmmss%.log"

goto exit

:usage
echo Usage: deploy [help] [-c filename]
